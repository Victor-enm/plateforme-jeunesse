...
const phase = ref('detach') // 'premise' | 'detach' | 'merge' | 'logo'

const iconMap = { E: EIcon, M: MIcon, C: CIcon, Q: QIcon }

const letters = [
  { letter: 'E', from: { x: '-30%', y: '10%' }, to: { x: '15%', y: '15%' } },
  { letter: 'M', from: { x: '20%', y: '10%' }, to: { x: '35%', y: '15%' } },
  { letter: 'C', from: { x: '-22%', y: '45%' }, to: { x: '15%', y: '40%' } },
  { letter: 'Q', from: { x: '30%', y: '45%' }, to: { x: '35%', y: '40%' } }
]
...
onMounted(() => {
  setTimeout(() => (phase.value = 'detach'), 1200) // 0 → 1.2 s
  setTimeout(() => (phase.value = 'merge'), 2200)  // 1.2 → 2.2 s
  setTimeout(() => (phase.value = 'logo'), 3200)   // 2.2 → 3.2 s
  setTimeout(() => emit('finished'), 4000)         // 3.2 → 4.0 s
})
</script>
  ...
  <transition name="fade">
    ...
    >
      <div class="relative flex h-72 w-80 items-center justify-center">
        ...
        <div
          v-if="phase === 'sentence' || phase === 'detach' "
          class="text-center text-2xl md:text-3xl font-medium text-violet"
        >
          <span>
            tre
          </span>
        ...
        <transition name="logo">
          <div v-if="phase === 'logo'" class="flex items-center justify-center">
            <Logo
              alt="Logo"
